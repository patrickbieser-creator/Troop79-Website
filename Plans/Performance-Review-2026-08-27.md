...
**Status:** All 18 items shipped 2026-08-27 (v1.111.0 → v1.114.0; each row marked DONE). Left for a later pass: ISR + tag cache for the force-dynamic public catalog pages (#18 note), and `'use cache'`/cacheComponents adoption. Patrick: "some forms take 3–4 s, signup clicks
...
| 10 | Audits (DONE v1.114.0) | `advancement/audits/page.tsx:72-77` | 7 check modules each re-read `scouts`, `rank_requirements`, and paginate `ledger_active` independently (~30–40 queries; +1 trip per 1000 ledger rows per check) | Doubles per 5k ledger rows | Load once, pass a shared snapshot into each `run(input)`; or SQL views. | M |
| 11 | Agenda tab (DONE v1.114.0) | `meeting-plan/load-input.ts:22-82` via `load-agenda.ts:52` | Opening Agenda runs the meeting-plan engine's 13-query fan-out incl. 4 `fetchAllRows` loops (`merit_badge_requirements` 1.7k rows, `mb_progress`, `ledger_active` twice) | Ledger-linear, weekly | `unstable_cache` the catalog half (ranks/MB reqs/skills) with a tag; make candidates a lazy client fetch on tray open. | M |
| 12 | Signup write path (DONE v1.114.0) | `events/[id]/actions.ts:173-306` | ~20 sequential awaits: two guard cookie reads + two `person_authz`, two `signup_entries` updates, per-slot claim read/delete/RPC, per-set unplace/place | O(slots × people) | Batch claim deletes `.in()`; one entry update; pass session through guards (item 1 covers the reads). | M |
