...
4. ~~**Proof via both gates.** Family password (household device recognition scopes the scout picker to their own kids) and the shared scout password (scout picks their name; leader review catches misuse).~~
   **SUPERSEDED (Patrick, 2026-08-06) — see Plans/Family-Identity-Auth.md Phase 0.** "Leader review catches misuse" was never actually load-bearing: the reviewer has no independent way to verify a scout's identity claim, and the shared `SCOUT_PASSWORD` login let any holder claim proof under any active scout's name. The scout path is closed entirely (not narrowed) until Tier 2-S (verified scout identity) ships. Only the family gate submits proof now.
