...
export default async function RosterPage({
  ...
  if (!actor?.capabilities.has("roster.manage")) {
    return (
      <div className={styles.gate}>
        The roster is available to adult leaders only.
      </div>
    );
  }
  ...
  const [
    ...
    supabase
      .from("people")
      .select(
        "id, display_name, primary_phone, guest_host_household_id, created_at",
      )
  ...
  const guestPeople = (guestPeopleRes.data ??
    []) as unknown as GuestPersonRow[];
  ...
  const guestRows = buildGuestTabRows({
    ...
    events: guestEvents,
    householdLabels: householdLabel,
    today,
  });
  ...
  const counts = TABS.map((t) => ({
    ...
    n:
      t.key === "guest"
        ? guestRows.length
        : directory.filter((p) => p.tab === t.key).length,
  }));
  ...
  const scoutIdsFor = (key: TabKey) =>
    new Set(
      directory
        .filter((p) => p.tab === key && p.scout_id)
        .map((p) => p.scout_id as string),
    );
  const tabScouts =
  ...
  // One global name search above the tabs (2026-08-27) — every tab, guests
  // included, normalized to one row shape; a hit deep-links to its tab.
  const searchRows = buildRosterSearchRows({
    directory,
    guests: guestRows,
    householdByPerson,
    householdLabel,
    ...
    <>
      ...
      <RosterSearch
        ...
        tabs={
          /* Shared TabStrip (Phase A, 2026-08-21), link mode — tab state stays
             ...
             never had. Inert while a search is active. */
          <TabStrip
      ...
      >
        {tab === "active_scout" && turning18.length > 0 && (
          <div className={styles.callout}>
            ...
            {turning18
              ...
              .join(" · ")}{" "}
            — record any outstanding sign-offs, then open the scout and use
            Promote to adult. At 18 they leave the scout roster and appear under
            Leaders or Adults.
          </div>
        )}
        ...
        {(tab === "adult" || tab === "leader") && inactiveAdults > 0 && (
          <div className={styles.callout}>
            <strong>{inactiveAdults} people are marked inactive.</strong> They
            stay on record for history but are not offered in the family signup
            picker. Open one and use Mark active to bring them back.
          </div>
        )}

        {tab === "guest" ? (
          <GuestsTable rows={guestRows} openGuestId={openScoutId} />
        ) : tab === "active_scout" || tab === "inactive_scout" ? (
          <ScoutsTable
            scouts={tabScouts}
            ranks={ranks}
