...
type Loaded = NonNullable<Awaited<ReturnType<typeof loadRoster>>>;
/** A loaded roster whose entry resolved — callers notFound() otherwise. */
export type RosterData = Loaded & { entry: NonNullable<Loaded['entry']> };

export async function loadRoster(signupId: number) {
...
/** The roster itself — everything under the event tab strip. Rendered by the
 *  standalone page AND inside the calendar workbench's Signup tab (?view=roster,
 *  2026-08-25), so a leader never loses the entry's head and tabs. */
export function RosterView({ data, signupId }: { data: RosterData; signupId: number }) {
