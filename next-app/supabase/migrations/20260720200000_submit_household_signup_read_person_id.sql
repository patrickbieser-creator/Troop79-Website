-- submit_household_signup + cancel_party_signup now READ person_id too.
--
-- Safe now, and only now (Fable sequencing review, 2026-07-20): every
-- signup_entries row has person_id populated — the write-side migration
-- (20260720180000) has been resolving it on every insert/update since it
-- landed, and the backfill (20260720190000, after the unmerge fix in
-- 20260720185000) closed the gap for every row written before that. A row
-- with person_id NULL reaching this lookup would be invisible to the new
-- OR-arm and get silently re-inserted — the exact bug this migration exists
-- to end — which is why this had to wait until that precondition held.
--
-- The existing-row lookup now matches on person_id OR any legacy column
-- (legacy kept one more release as a fallback, per the sequencing review, for
-- any code path that still doesn't resolve person_id — e.g. adult_name-only
-- entries). This is what makes two submissions for the SAME PERSON through
-- DIFFERENT legacy identity paths (a leader row one call, a scout_parents row
-- the next — the literal shape of D-042) resolve to ONE row instead of
-- tripping signup_entries_person_uniq on a second insert.
...
begin
  ...
  loop
    ...
    -- THE CHANGE: person_id now leads the OR. Legacy columns stay as a
    -- fallback for one more release (an adult_name-only entry has no
    -- person_id to match on at all).
...
-- cancel_party_signup gains a person_id array, matched alongside the legacy
-- arrays — additive, so existing callers passing only the legacy arrays are
-- unaffected. The old 6-arg signature must be dropped explicitly: adding a
-- parameter changes the signature, so CREATE OR REPLACE would otherwise
-- create a second overload instead of replacing the first, and PostgREST
-- can't then pick between them (PGRST203).
drop function if exists public.cancel_party_signup(bigint, text, bigint, text[], bigint[], text[]);
    ...
    and (
      case
        when p_household_id is not null then household_id = p_household_id
          ...
          (person_id is not null and person_id = any (p_person_ids))
          or (scout_id is not null and scout_id = any (p_scout_ids))
