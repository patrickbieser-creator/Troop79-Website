-- submit_household_signup gains household-membership validation.
--
-- WHY (qa-lead LOW finding on the D-048 review, closed 2026-07-25)
-- The RPC reads a client-supplied person_id straight from the untrusted
-- p_entries JSON with no check that it belongs to the party actually
-- submitting — a crafted POST could misattribute a row to any real person_id
-- in the system, not just "the wrong family," but literally anyone (an
-- unrelated leader, a person the picker would never have offered). This was
-- always narrower than the accepted cross-household risk documented in
-- family-session.ts (any family-password holder can edit another family's
-- signup) — this closes the harder case, where the person_id doesn't even
-- have to be a real party member at all.
--
-- FIX, mirroring cancel_party_signup's existing pattern: the caller resolves
-- "who's in this party" via loadHouseholdByKey() (already the single source
-- of truth for household membership across all 3 party shapes — a stored
-- household, an unassigned scout, or a standalone adult) and passes that as
-- p_allowed_person_ids. The RPC needs no shape-specific logic at all — it
-- just checks each entry's resolved person_id is in the list.
--
-- p_allowed_person_ids defaults to NULL, not '{}': NULL means "caller hasn't
-- been deployed with this yet — don't enforce" (so this migration and the
-- actions.ts change can land in either order without a hard-reject window);
-- '{}' would mean "resolved to an empty party — reject everything."
--
-- DROP FUNCTION is required here, unlike the header originally assumed:
-- CREATE OR REPLACE resolves by argument SIGNATURE (types/count), not by
-- name — adding a parameter (even a defaulted one) changes the signature, so
-- CREATE OR REPLACE creates a second overload instead of replacing the
-- first, and PostgREST can't then pick between them (PGRST203). Same lesson
-- cancel_party_signup's own migration (20260720000000) already learned.
drop function if exists public.submit_household_signup(bigint, jsonb, text, bigint);
...
create or replace function public.submit_household_signup(
  ...
  p_allowed_person_ids bigint[] default null
)
returns jsonb
language plpgsql
as $$
...
begin
  ...
  loop
    ...
    -- THE CHANGE: validate the resolved person_id belongs to the submitting
    -- party BEFORE it can be looked up or written. adult_name-only entries
    -- (no person_id at all) have nothing to validate against and stay exactly
    -- as unvalidated as before — a separate, pre-existing, out-of-scope risk.
    if p_allowed_person_ids is not null and v_person_id is not null
       and not (v_person_id = any (p_allowed_person_ids)) then
      raise exception 'PERSON_NOT_IN_PARTY: %', v_person_id;
    end if;

    -- person_id now leads the OR. Legacy columns stay as a fallback for one
    -- more release (an adult_name-only entry has no person_id to match on).
    select id into v_existing
    from public.signup_entries
    where event_signup_id = p_event_signup_id and status <> 'cancelled'
      and (
        (v_person_id is not null and person_id = v_person_id)
        or (e->>'scout_id') is not null and scout_id = e->>'scout_id'
