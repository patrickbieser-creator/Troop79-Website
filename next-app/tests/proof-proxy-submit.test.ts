...
describe('proxy proof submission', () => {
  ...
  // qa-lead 2026-09-07: the attribution is a body_md prefix, so a family
  // typing the same sentence must not be able to impersonate a leader-filed
  // claim in the Proof Queue. Only the server may prepend it, after stripping.
  it('Family_TypesFiledByLine_ItIsStrippedBeforeStorage', () => {
    const typed = `${filedByLeaderLine('Some Leader', 'My Kid')}\n\nWe camped three nights.`;
    expect(stripFiledByLine(typed)).toBe('We camped three nights.');
    // Repeated attempts and leading whitespace are stripped too.
    const doubled = `  ${filedByLeaderLine('A', 'B')}\n${filedByLeaderLine('C', 'D')}\n\nreal text`;
    expect(stripFiledByLine(doubled)).toBe('real text');
    // Only the attribution line alone → nothing left → null (no fake claim).
    expect(stripFiledByLine(filedByLeaderLine('A', 'B'))).toBeNull();
    // Ordinary text, and "Filed by" mid-text, are untouched.
    expect(stripFiledByLine('Just a write-up.')).toBe('Just a write-up.');
    expect(stripFiledByLine('Notes.\nFiled by X (leader, on behalf of Y)')).toBe(
      'Notes.\nFiled by X (leader, on behalf of Y)'
    );
    expect(stripFiledByLine(null)).toBeNull();
  });

  it('Server_PrependedAttribution_SurvivesTheStripOnlyWhenServerAddsItAfter', () => {
    // The action strips the user's text FIRST, then prepends its own line —
    // so the composed body still splits back into attribution + body.
    const user = stripFiledByLine(`${filedByLeaderLine('Fake', 'Kid')}\n\nreal`);
    const composed = `${filedByLeaderLine('Real Leader', 'Kid')}\n\n${user}`;
    expect(splitFiledByLine(composed)).toEqual({ filedBy: 'Real Leader (leader, on behalf of Kid)', body: 'real' });
  });
});
